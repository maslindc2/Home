...
export async function fetchWeather() {
	// If the zip code env is undefined send error message to front that zip code is undefined
	if (!ZIP_CODE) {
		var jsonString =
			'{"current":{"temp_f": 0,"feelslike_f":0, "condition":{"text":"Zip Code Env undefined"}}}';
		var jsonObject = JSON.parse(jsonString);
		return jsonObject;
		// If the weather api key is undefined but we have the OpenWeather map api key fetch the current weather from there
	} else if (!WEATHER_API_KEY && OPENWEATHER_MAP_API_KEY) {
		return fetchWeatherFromBackupAPI();
		// Otherwise we have all three env variables, attempt to fetch from weather api
	} else {
